	...
	for {
		...
		if isValidName && isValidEmail && isValidTicketNumber {
			...
			if remainingTickets == 0 {
				// end program
				fmt.Println("Our congerence is booked out. Come back next year.")
				break
			}
		} else {
			if !isValidName {
			...
			if !isValidTicketNumber {
				fmt.Printf("We only have %v tickets remaining, can't book %v tickets.\n", remainingTickets, userTickets)
				fmt.Println("Your entered ticket is invalid.")
			}
		}
